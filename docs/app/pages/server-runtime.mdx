...
### `readRawBody(event, encoding)`: Read the raw request body
...
### `readBody(event, options)`: Read the request body as JSON
...
### `readValidatedBody(event, validate)`: Read the request body and validate it
...
### `readMultipartFormData(event)`: Read the request body as multipart/form-data
...
### `readFormData(event)`: Read the request body as FormData
...
### `getRequestWebStream(event)`: Get the request body as a stream
...
### `getRequestHost(event, opts)`: Get the request host
...
### `getRequestProtocol(event, opts)`: Get the request protocol
...
### `getRequestURL(event, opts)`: Get the request URL
...
### `getRequestIP(event, opts)`: Get the request IP
...
### `isPreflightRequest(event)`: Check if the request is a CORS preflight request
...
### `getWebRequest(event)`: Get the web request
...
### `parseCookies(event)`: Parse the request cookies
...
###  `getCookie(event, name)`: Get a cookie value by name
...
###  `setCookie(event, name, value, serializeOptions)`: Set a cookie value by name
...
###  `deleteCookie(event, name, options)`: Delete a cookie by name
...
### `fetchWithEvent(event, req, init, options)`: Fetch with event
...
### `getValidatedQuery(event, validate)`: Get the validated query
...
```ts
export function sendProxy(event: HTTPEvent, options: {
    target: string | URL;
    [key: string]: any;
}): Promise<void>;
```
...
```ts
export function proxyRequest(event: HTTPEvent, options: {
    target: string | URL;
    [key: string]: any;
}): Promise<void>;
```

/**
 * @license React
 * react-server-dom-vite-client.node.development.js
 *
...
function resolveClientReference(bundlerConfig, metadata) {
  var _metadata$split = metadata.split('#'),
      specifier = _metadata$split[0],
      name = _metadata$split[1];
...
function preloadModule(metadata) {
  ...
  var exisitngModule = __vite__.cached(metadata.specifier);

  if (exisitngModule) {
    if (exisitngModule.status === 'fulfilled') {
      return null;
    } else if (exisitngModule) {
      return exisitngModule;
    }
  } else {
    // $FlowFixMe[unsupported-syntax]
    var modulePromise = __vite__.import(metadata.specifier); // modulePromise.then(
    //   value => {
...
function requireModule(metadata) {
  if (__vite__.require) {
    return __vite__.require(metadata.specifier)[metadata.name];
  }

  var moduleExports; // We assume that preloadModule has been called before, which
  // should have added something to the module cache.

  var modulePromise = __vite__.cached(metadata.specifier);

  if (modulePromise.status === 'fulfilled') {
    moduleExports = modulePromise.value;
  } else if (modulePromise.status === 'pending') {
    throw modulePromise;
  } else {
    throw modulePromise.reason;
  }
...
function createFromNodeStream(stream, moduleMap, moduleBaseURL) // TODO: Used for preloading hints
{

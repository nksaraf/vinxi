...
function resolveClientReference(bundlerConfig, metadata) {
  const _metadata$split = metadata.split('#'),
...
function preloadModule(metadata) {
  const existingPromise = __vite__.moduleGraph.getModuleById(metadata.specifier);
  ...
  if (existingPromise) {
    ...
    const modulePromise = __vite__.ssrLoadModule(metadata.specifier);
...
function requireModule(metadata) {
  ...
  const promise = __vite__.moduleGraph.getModuleById(metadata.specifier);

 ...
 * @returns
 */
async function resolveClientImport(vite, specifier, parentURL) {
...
async function parseExportNamesInto(vite, ast, names, parentURL) {
...
function decorateExports({ code, id, ast, runtime, hash, options }) {
...
function shimExportsPlugin({ runtime, hash, pragma, apply, onModuleFound }) {
	return {
		name: "shim-exports",
...
function decorateExportsPlugin({
	runtime,
	hash,
	pragma,
	apply,
...
export function transformReferences({
	...
	const plugins = [
		shimExportsPlugin({
			...
			onModuleFound: onServerReference,
			hash: hash,
			apply: (code, id, options) => {
				return !options.ssr;
			},
		...
		shimExportsPlugin({
			...
			onModuleFound: onClientReference,
			hash: hash,
			apply: (code, id, options) => {
				return options.ssr;
			},
	...
	return {
		...
		transform(code, id, options) {
			...
			for (var plugin of plugins) {
				if (plugin.apply(code, id, opts)) {
					return plugin.transform(code, id, opts);
				}
			}
		},
	};
